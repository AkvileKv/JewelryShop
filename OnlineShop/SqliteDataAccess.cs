...
namespace OnlineShop
{
    public class SqliteDataAccess
    {
        ...
        {
            using (IDbConnection cnn = new SQLiteConnection(LoadConnectionString())) //connection to database, protects from error
            {
        ...
        public static void SaveItem(Item item) //jeo pabraukti, 
        {
    ...
    /*
            public static void UsernameAndPassword (Customer customer) //neveikia, gal truksta open ir close (locked DB)
            {
                using (IDbConnection cnp = new SQLiteConnection(LoadConnectionString()))

                    cnp.Execute("select * from Customer where Username =@Username and Password = @Password", customer);
            }
